...
use \Bitrix\ImConnector\Connectors\Instagram;
	...
	private function query($command, $result)
	{
			...
			$data = array();
                //nmcuong log message
                        $arLog = array(
                                    ...
                                    "LINE"=>$this->line,
                                    "COMMAND"=>$command,
                                    "MODE"=> Library::LOG_MODE_OUTPUT,
                                    "STEP"=> Library::LOG_STEP_9,
                                    "CODE"=>"Output_query",
                                    "MESSAGE"=>"Output_query",
                                    "ERROR"=> Library::LOG_STATUS_NON_ERROR
                                );
                        LogDB::write($arLog, __FILE__, __METHOD__, $data);
		...
		else
		{
			...
			$params = Encoding::convertEncoding($params, SITE_CHARSET, 'UTF-8');
                        error_log(json_encode($this->controllerUrl), 3, "/var/log/bitrix/my.log");
                        error_log(json_encode($params), 3, "/var/log/bitrix/my.log");
                        error_log(json_encode("################"), 3, "/var/log/bitrix/my.log");
                        
                        if($this->connector == 'botframework.skype'){
                            $this->controllerUrl = "https://demo1.ipcc.space/ext/botman/botframework_reply.php";
                        }
                        if($this->line == 12){
                            $this->controllerUrl = "https://demo1.ipcc.space/ext/botman/chanel_config_receive.php";
                        }
                        //nmcuong log message
                        $arLog = array(
                                    "CONNECTOR"=>$this->connector,
                                    ...
                                    "STEP"=> Library::LOG_STEP_9,
                                    "CODE"=>"Output_query",
                                    "MESSAGE"=>$this->controllerUrl,
                                    "ERROR"=> Library::LOG_STATUS_NON_ERROR
                                );
                        LogDB::write($arLog, __FILE__, __METHOD__, $params);
			...
			if($waitResponse && $result->isSuccess() && !in_array(strtolower($params['BX_COMMAND']), $this->listCommandNotWaitResponse))
			{
				try
				{
                                    ...
                                    $arLog = array(
                                        ...
                                        "MODE"=> Library::LOG_MODE_OUTPUT,
                                        "STEP"=> Library::LOG_STEP_9,
                                        "CODE"=>"Output_query_response",
                                        "MESSAGE"=>$this->controllerUrl,
                                        "ERROR"=> Library::LOG_STATUS_NON_ERROR
                                    );
                                    LogDB::write($arLog, __FILE__, __METHOD__, $request);
				}
				catch (\Exception $e)
				{
                                    ...
                                    $arLog = array(
                                        ...
                                        "CODE"=>"Output_query_response",
                                        "MESSAGE"=>$this->controllerUrl,
                                        "ERROR"=> Library::LOG_STATUS_ERROR
                                    );
                                    LogDB::write($arLog, __FILE__, __METHOD__, $result->getErrors());
				}
			}
		}
